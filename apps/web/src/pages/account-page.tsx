...
function ProviderKeyField({
  ...
  async function testConnection() {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`${meta.name} key is valid`);
      ...
      <main className="flex-1 flex items-start justify-center py-10 px-4">
        <div className="w-full max-w-lg">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Keys are used directly from your browser — never sent to Archlet servers.
          </p>

          {/* XSS warning */}
          <div className="flex gap-2 items-start rounded-md border border-amber-200 bg-amber-50 dark:bg-amber-950 dark:border-amber-800 px-3 py-2.5 mb-6 text-xs text-amber-700 dark:text-amber-300">
            <AlertTriangle size={13} className="shrink-0 mt-0.5" />
            <span>
              Keys are stored in browser <strong>localStorage</strong> as plain text.
              Any XSS vulnerability on this page could expose them.
